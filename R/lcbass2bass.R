...
#' @param offset This value is added to the intercept of the resulting model.
#' @param yy The data vector. When unspecified, y is taken as the linear combination of responses for the individual bass models.
...
#'
#' @export
basslc2bass <- function(mod_list, weights=rep(1, length(mod_list)), offset=0, yy=NULL, mcmc.use=NULL){
  if(!is.list(mod_list)){
  ...
  if(is.null(yy)){
    y_est <- offset + weights[1] * mod_curr$y
  }
  ...
  if(nbassmodels > 1){
    for(i in 2:nbassmodels){
      ...
      if(is.null(yy)){
        y_est <- y_est + weights[i] * mod_curr$y
      }
    }
  }
  ...
  for(mm in seq_along(mcmc.use)){
    ...
    if(new_model_flag){
      ...
      is_duplicate_model <- any(apply(modelFull, 1,
