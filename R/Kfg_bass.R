#' Estimate the Co-Zahm Matrix with BASS
#'
#' Closed form estimator of the Co-Zahm matrix using a BASS model
#'
#' @param mod1 a fitted functional BASS model, or a list of univariate bass models for function 1.
#' @param mod2 a fitted functional BASS model, or a list of univariate bass models for function 2.
...
#'
#' @export
Kfg_bass <- function(mod1, mod2,
                    prior=NULL, prior_func=function(tt) dunif(tt),
  ...
  if(is.null(func.use)){
    ...
    if(class(mod1) == "bassBasis"){
      nfunc <- nrow(mod1$dat$basis)
      func.use <- seq(0, 1, length.out=nfunc)
    }
    if("bass" %in% class(mod1)){
      if(mod1$func){
        nfunc <- nrow(mod1$xxfunc)
        func.use <- mod1$xx.func
      }
    }
  }else{
  ...
  # Calculate Cfg Matrices
  res <- Cfg_bass(mod1, mod2, prior, mcmc.use, use_native_scale, func.use, func.true, pca_method, verbose)
